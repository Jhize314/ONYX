...
export namespace FsContentRequest {
	export const type: RequestType<{ uri: string; encoding?: string }, string, any> =
		new RequestType('fs/content');
}

export namespace FsStatRequest {
	export const type: RequestType<string, FileStat, any> =
		new RequestType('fs/stat');
}
...
export function serveFileSystemRequests(client: BaseLanguageClient, runtime: Runtime) {
	client.onRequest(FsContentRequest.type, (param: { uri: string; encoding?: string }) => {
		...
		if (uri.scheme === 'file' && runtime.fs) {
			return runtime.fs.getContent(param.uri, param.encoding);
		}

		return workspace.fs.readFile(uri).then(buffer => {
			const arrayBuffer = new ArrayBuffer(buffer.byteLength);
			new Uint8Array(arrayBuffer).set(buffer);
			return new runtime.TextDecoder(param.encoding).decode(arrayBuffer);
		});
	});
