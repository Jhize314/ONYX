...
suite('vscode API - workspace', () => {
	...
	test('openTextDocument, actual casing first', async function () {
		...
		fs.writeFile(uriOne, textEncoder.encode('one'), { create: true, overwrite: true });
		fs.writeFile(uriTwo, textEncoder.encode('two'), { create: true, overwrite: true });
	...
	test('Support creating binary files in a WorkspaceEdit', async function (): Promise<any> {
		...
		const data = textEncoder.encode('Hello Binary Files');
	...
	test('encoding: decode', async function () {
		...
		// without setting
		assert.strictEqual(await vscode.workspace.decode(textEncoder.encode('Hello World'), uri), 'Hello World');
		assert.strictEqual(await vscode.workspace.decode(textEncoder.encode('Hellö Wörld'), uri), 'Hellö Wörld');
		...
		assert.strictEqual(await vscode.workspace.decode(textEncoder.encode('Hello World'), uri, { encoding: 'foobar123' }), 'Hello World');
